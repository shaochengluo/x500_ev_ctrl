...
# ------------ ENU <-> NED (world) and FLU -> FRD (body) ------------
C_ENU2NED = np.array([[0, 1, 0],
                      [1, 0, 0],
                      [0, 0,-1]], dtype=float)

# Body-frame flip: Vicon subject is FLU, PX4 expects FRD
C_FLU2FRD = np.diag([1, -1, -1])   # 180° about X (Y,Z sign flip)

def enu_pos_to_ned(p_enu):
    # world frame only (no body flip for position)
    x_e, y_e, z_e = p_enu
    return np.array([y_e, x_e, -z_e], dtype=float)

def enu_quat_to_ned_frd(q_enu_xyzw):
    """
    Convert Vicon quaternion (ENU world, FLU body) to PX4 (NED world, FRD body).
    Return as [w, x, y, z] because VehicleOdometry expects that order.
    """
    x, y, z, w = q_enu_xyzw             # ENU/FLU in [x y z w]
    R_enu_flu = quat_to_rotmat(x, y, z, w)

    # Change world basis ENU->NED and body basis FLU->FRD:
    # R^NED_FRD = C_w * R^ENU_FLU * C_b^T
    R_ned_frd = C_ENU2NED @ R_enu_flu @ C_FLU2FRD.T

    xN, yN, zN, wN = rotmat_to_quat(R_ned_frd)  # returns [x y z w]
    return [float(wN), float(xN), float(yN), float(zN)]
...
class ViconToPX4EV(Node):
    ...
    def publish_ev(self):
        ...
        qw, qx, qy, qz = enu_quat_to_ned_frd(q_enu)
